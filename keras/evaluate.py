...
def evaluate(cli_args):
	...
	probabilities = model.predict_generator(data_generator.get_data(should_shuffle=False, is_prediction=True),val_samples=data_generator.get_num_files(),nb_worker=1,max_q_size=config["batch_size"], pickle_safe=True)
