...
def download(language, source, source_name, source_type):
	...
	if source_type == "playlist":
		...
		command = """youtube-dl -i --download-archive {} --max-filesize 50m --no-post-overwrites --max-downloads {} --extract-audio --audio-format wav {} -o "{}/%(title)s.%(ext)s" """.format(playlist_archive, args.max_downloads, source, output_path_raw)
